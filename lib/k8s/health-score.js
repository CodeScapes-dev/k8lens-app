import { isOomKilled, isStuckPending, podFailureReason, totalRestarts } from "./diagnostics-common.js";
...
export function computeHealthScore(resourceType, data) {
  ...
  switch (resourceType) {
    case "pod": {
      ...
      if (isOomKilled(statuses, events)) fire("OOMKilled", "Pod was killed due to memory limit being exceeded", 30);
      ...
      if (isStuckPending(pod)) fire("StuckInPending", "Pod has been in Pending state for more than 5 minutes", 30);

      const failure = podFailureReason(pod);
      if (failure === "CrashLoopBackOff") fire("CrashLoopBackOff", "A container keeps crashing and being restarted", 40);
      else if (failure === "ImagePullBackOff" || failure === "ErrImagePull") fire("ImagePullFailure", "The container image can't be downloaded", 40);
      else if (["CreateContainerConfigError", "CreateContainerError", "InvalidImageName"].includes(failure)) fire("ContainerStartError", "A container can't be created, usually a configuration problem", 40);
      else if (failure === "Evicted") fire("Evicted", "The pod was evicted from its node", 50);
      else if (failure === "Failed") fire("PodFailed", "The pod ended in a failed state", 50);
      else if (failure === "NotReady") fire("NotReady", "A container is running but failing its readiness check", 20);
      break;
    }
    ...
    case "daemonset": {
      ...
      // DaemonSets have no spec.replicas or spec.strategy; they schedule one pod per node.
      const isDaemonSet = resourceType === "daemonset";
      const desired = isDaemonSet ? (workload?.status?.desiredNumberScheduled ?? 0) : (workload?.spec?.replicas ?? 0);
      const available = isDaemonSet
        ? (workload?.status?.numberAvailable ?? workload?.status?.numberReady ?? 0)
        : (workload?.status?.availableReplicas ?? workload?.status?.readyReplicas ?? 0);
      const strategy = (isDaemonSet ? workload?.spec?.updateStrategy : workload?.spec?.strategy) ?? {};
