import { isCrashing, podFailureReason } from "./diagnostics-common.js";
...
export function buildUnhealthyWorkloads(pods = [], deployments = [], statefulSets = [], daemonSets = []) {
  ...
  pods.forEach((pod) => {
    ...
    if (failure && failure !== "NotReady") {
      items.push({ kind: "Pod", name, namespace, reason: failure, age, severity: "error" });
    } else if (phase === "Pending") {
      items.push({ kind: "Pod", name, namespace, reason: "Pending", age, severity: "warning" });
    } else if (failure === "NotReady") {
      items.push({ kind: "Pod", name, namespace, reason: "NotReady", age, severity: "warning" });
    }
  });
  ...
  // Sort: errors first, then warnings (stable within each group)
