...
export const statefulSetColumns = [
  ...
  },
  {
    ...
    accessorFn: (row) => row.spec?.updateStrategy?.type ?? "—",
...
export const daemonSetColumns = [
  ...
  nsCol(),
  {
    ...
    meta: { mono: true, align: "right", w: "0.6fr", sortable: true },
  },
  {
    ...
    cell: (info) => {
      const row = info.row.original;
      const desired = row.status?.desiredNumberScheduled ?? 0;
      const ready = info.getValue();
      return <span style={{ color: ready < desired ? "var(--kl-warn)" : "var(--kl-text)" }}>{ready}</span>;
    },
  },
  {
    id: "available", header: "Available",
    accessorFn: (row) => row.status?.numberAvailable ?? 0,
    meta: { mono: true, align: "right", w: "0.7fr" },
  },
  {
    id: "upToDate", header: "Up-to-date",
