...
test("AC4b: selector matching only some nodes does not claim a mismatch", () => {
  ...
  assert.match(d.rootCause, /isn't enough free cpu/);
...
const ANTI_AFFINITY_MESSAGE =
  "0/2 nodes are available: 1 node(s) didn't satisfy existing pods anti-affinity rules, 1 node(s) had untolerated taint(s). no new claims to deallocate, preemption: 0/2 nodes are available: 1 Preemption is not helpful for scheduling, 1 node(s) didn't satisfy existing pods anti-affinity rules.";

test("pod anti-affinity from the scheduler is explained in plain words with the rolling-update hint", () => {
  const [d] = podDiag(f.pendingTolerationPod, { nodes: f.allTaintedNodes.map((n) => ({ ...n, spec: { taints: [] } })), events: [f.failedSchedulingEvent(ANTI_AFFINITY_MESSAGE)] });
  assert.match(d.rootCause, /1 machine already runs a pod that this pod's anti-affinity rule says it must stay away from/);
  assert.doesNotMatch(d.rootCause, /Its scheduler says/);
  assert.match(d.evidence.join("\n"), /didn't satisfy existing pods anti-affinity rules/);
  assert.match(d.remediation.join("\n"), /maxUnavailable to 1/);
  assert.match(d.remediation.join("\n"), /podAntiAffinity/);
});

test("storage, cordon and pod-affinity scheduler messages are each interpreted", () => {
  const explain = (message) => podDiag(f.pendingTolerationPod, { nodes: f.plainNodes.map((n) => ({ ...n, spec: {} })), events: [f.failedSchedulingEvent(message)] })[0];
  assert.match(explain("0/2 nodes are available: pod has unbound immediate PersistentVolumeClaims.").remediation.join("\n"), /Persistent Volume Claims/);
  assert.match(explain("0/2 nodes are available: 2 node(s) were unschedulable.").rootCause, /2 machines are cordoned/);
  assert.match(explain("0/2 nodes are available: 2 node(s) didn't match pod affinity rules.").remediation.join("\n"), /podAffinity/);
  assert.match(explain("0/2 nodes are available: 1 Insufficient memory, 1 Insufficient cpu.").rootCause, /enough free memory and cpu/);
});

test("an unrecognised scheduler message is still shown as is", () => {
  const [d] = podDiag(f.pendingTolerationPod, { nodes: f.plainNodes.map((n) => ({ ...n, spec: {} })), events: [f.failedSchedulingEvent("0/2 nodes are available: something new.")] });
  assert.match(d.rootCause, /Its scheduler says: "0\/2 nodes are available: something new\."/);
});

test("without node access the node-level scheduler reasons are interpreted too", () => {
  const [d] = podDiag(f.pendingSelectorPod, { nodesError: true, events: [f.failedSchedulingEvent("0/2 nodes are available: 2 node(s) had untolerated taint(s).")] });
  assert.match(d.rootCause, /2 machines are reserved for other workloads/);
  assert.match(d.evidence.join("\n"), /Couldn't check the cluster's machines/);
});
