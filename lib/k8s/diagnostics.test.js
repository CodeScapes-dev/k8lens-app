...
test("AC1: missing ConfigMap key names the ConfigMap and key", () => {
  ...
  assert.equal(d.issue, "CrashLoopBackOff");
...
test("AC4: unmatched nodeSelector names key/value and node counts, offers both fixes", () => {
  ...
  assert.equal(d.issue, "Pod stuck in Pending");
  assert.equal(d.severity, "Critical");
  assert.match(d.rootCause, /nodeSelector disktype=ssd matches 0 of 2 nodes/);
  const r = d.remediation.join("\n");
  assert.match(r, /kubectl label node <node-name> disktype=ssd/);
  assert.match(r, /Remove or relax "disktype: ssd"/);
});
...
test("AC4b: selector matching only some nodes does not claim a mismatch", () => {
  ...
  assert.doesNotMatch(d.rootCause, /No node satisfies/);
});
...
test("AC5: untolerated taint names key/value/effect and node count, offers both fixes", () => {
  ...
  assert.match(d.rootCause, /2 of 2 nodes carry taint dedicated=gpu:NoSchedule/);
  const r = d.remediation.join("\n");
  assert.match(r, /Add a toleration for dedicated=gpu:NoSchedule/);
...
test("required nodeAffinity mismatch is reported per expression", () => {
  ...
  assert.match(d.rootCause, /nodeAffinity "zone In \[us-east-1a\]" matches 0 of 2 nodes/);
});
...
test("AC9: missing node access degrades with an explicit message", () => {
  ...
  assert.match(text(d), /Couldn't verify against live node state/);
  assert.match(d.rootCause, /0\/1 nodes are available/);
  const [noNodes] = podDiag(f.pendingSelectorPod);
  assert.match(text(noNodes), /Couldn't verify against live node state/);
});
...
test("multi-container pod reports only the crashing container", () => {
  ...
  assert.match(ds[0].rootCause, /container "sidecar"/);
});
...
test("AC7: diagnostics.js makes no network calls", () => {
  const src = readFileSync(new URL("./diagnostics.js", import.meta.url), "utf8");
  assert.doesNotMatch(src, /\bfetch\s*\(|XMLHttpRequest|WebSocket|https?:\/\/|^\s*import /m);
});
