...
export function diagnoseStorageClass(data, now = Date.now()) {
  ...
  // With WaitForFirstConsumer a claim legitimately stays Pending until a pod uses it, so only count claims a pod is waiting on.
  const waitsForConsumer = sc.volumeBindingMode === "WaitForFirstConsumer";
  const inUse = new Set(data?.claimsInUse ?? []);
  const waiting = claims.filter((c) => c.status?.phase === "Pending" && isStuck(c, now) && (!waitsForConsumer || inUse.has(key(c))));
  if (waiting.length > 0) {
