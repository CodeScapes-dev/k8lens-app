...
// Plain-language readings of the scheduler's FailedScheduling message. Node label/taint causes are computed
// structurally from live nodes when possible, so they are only interpreted here when that data is unavailable.
const SCHEDULER_REASONS = [
  {
    re: /(?:(\d+) node\(s\) )?didn't satisfy existing pods anti-affinity rules|didn't match pod anti-affinity rules/i,
    plain: (m) => `${m[1] ? machines(m[1]) : "Some machines"} already ${m[1] === "1" ? "runs" : "run"} a pod that this pod's anti-affinity rule says it must stay away from.`,
    fix: [
      "Add a machine so the pods can be spread out, or loosen the pod's podAntiAffinity (for example use a preferred rule instead of a required one).",
      "If this is a rolling update with no spare machine, the new pod can't start next to the old one. Let the old pod go first (set maxUnavailable to 1) or scale down and back up.",
    ],
  },
  {
    re: /(?:(\d+) node\(s\) )?didn't match pod affinity rules/i,
    plain: (m) => `${m[1] ? machines(m[1]) : "Some machines"} ${m[1] === "1" ? "doesn't" : "don't"} run a pod that this pod's affinity rule says it must run next to.`,
    fix: ["Make sure the pods it must run next to are running, or loosen the pod's podAffinity rule."],
  },
  {
    re: /Insufficient ([\w-]+)/gi,
    plain: (_m, all) => `There isn't enough free ${[...new Set(all.map((x) => x[1].toLowerCase()))].join(" and ")} on the machines.`,
    fix: ["Lower this pod's resource requests, free up capacity by scaling down other workloads, or add machines."],
  },
  {
    re: /unbound immediate PersistentVolumeClaims|didn't find available persistent volumes to bind|persistentvolumeclaim "[^"]*" not found/i,
    plain: () => "A storage claim this pod needs isn't ready yet.",
    fix: ["Open the claim under Storage → Persistent Volume Claims and check that it is Bound."],
  },
  {
    re: /volume node affinity conflict/i,
    plain: () => "The pod's volume can only be used on certain machines, and none of them can take the pod.",
    fix: ["Check where the volume lives and make sure a suitable machine is available."],
  },
  {
    re: /(\d+) node\(s\) were unschedulable/i,
    plain: (m) => `${machines(m[1])} ${m[1] === "1" ? "is" : "are"} cordoned and won't accept new pods.`,
    fix: ["If maintenance is finished, uncordon them: kubectl uncordon <node-name>"],
  },
  {
    re: /(\d+) node\(s\) had untolerated taint/i,
    plain: (m) => `${machines(m[1])} ${m[1] === "1" ? "is" : "are"} reserved for other workloads (tainted) and this pod isn't allowed on ${m[1] === "1" ? "it" : "them"}.`,
    fix: ["Add a toleration for the taint to the pod, or remove or change the taint."],
    nodeRule: true,
  },
  {
    re: /(\d+) node\(s\) didn't match Pod's node affinity\/selector/i,
    plain: (m) => `${machines(m[1])} ${m[1] === "1" ? "doesn't" : "don't"} match the pod's nodeSelector or node affinity.`,
    fix: ["Label a machine so it matches, or loosen the pod's nodeSelector or node affinity."],
    nodeRule: true,
  },
];

function explainSchedulingMessage(message, { includeNodeRules }) {
  if (!message) return [];
  const out = [];
  for (const reason of SCHEDULER_REASONS) {
    if (reason.nodeRule && !includeNodeRules) continue;
    const all = reason.re.global ? [...message.matchAll(reason.re)] : null;
    const m = all ? all[0] : message.match(reason.re);
    if (m) out.push({ plain: reason.plain(m, all), fix: reason.fix });
  }
  return out;
}

const schedulerRootCause = (message, explained) =>
  explained.length > 0
    ? `Kubernetes can't find a machine for this pod. ${explained.map((e) => e.plain).join(" ")}`
    : `Kubernetes hasn't been able to start this pod. Its scheduler says: "${message}"`;
...
function diagnosePending(pod, events, nodes, nodesError, now) {
  ...
  if (nodesError || !Array.isArray(nodes) || nodes.length === 0) {
    const explained = explainSchedulingMessage(schedMessage, { includeNodeRules: true });
    return {
      ...base,
      severity: "Warning",
      rootCause: schedMessage
        ? schedulerRootCause(schedMessage, explained)
      ...
      remediation: [
        ...new Set(explained.flatMap((e) => e.fix)),
  ...
  const explained = explainSchedulingMessage(schedMessage, { includeNodeRules: false });
  return {
    ...base,
    severity: "Warning",
    rootCause: schedMessage
