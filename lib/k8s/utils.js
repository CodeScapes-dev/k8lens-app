...
// Absolute date/time in the given IANA timezone. `parts`: "datetime" (with seconds and zone), "date", "clock" (HH:MM) or "time" (HH:MM:SS).
const DATE_TIME_PARTS = {
  datetime: { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit", timeZoneName: "short" },
  date: { year: "numeric", month: "short", day: "numeric" },
  clock: { hour: "2-digit", minute: "2-digit" },
  time: { hour: "2-digit", minute: "2-digit", second: "2-digit" },
};

export function formatDateTime(timestamp, timezone = "UTC", parts = "datetime") {
  if (!timestamp) return "—";
  const date = new Date(timestamp);
  if (Number.isNaN(date.getTime())) return "—";
  try {
    return new Intl.DateTimeFormat("en-US", { timeZone: timezone, ...DATE_TIME_PARTS[parts] }).format(date);
  } catch {
    return date.toLocaleString();
  }
}
