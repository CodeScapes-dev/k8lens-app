...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("apps", "statefulsets", { listParams, namespace: nsFilter === "all" ? undefined : nsFilter });
  const { data: metricsData } = useMetrics("/api/k8s/metrics/workloads?kind=statefulsets");
  ...
  const columns = React.useMemo(() => [...statefulSetColumns, ...metricsColumns], [metricsColumns]);
  ...
  const kubectlCmd = nsFilter === "all"
    ...
    : `kubectl get statefulsets -n ${nsFilter}`;

  return (
    <div className="px-4 sm:px-6 py-5">
      <PageHeader title="StatefulSets" count={pagination?.totalItems} subtitle="apps/v1 · all namespaces" kubectlCmd={kubectlCmd} resourceKey="statefulsets"><StatusSummary data={data} /></PageHeader>
