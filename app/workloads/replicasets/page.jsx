...
import { replicaSetColumns } from "@/lib/k8s/columns/apps";
...
function StatusSummary({ data }) {
  const c = React.useMemo(() => {
    ...
    for (const d of data) { const des = d.spec?.replicas ?? 0; if (des === 0 || (d.status?.readyReplicas ?? 0) >= des) r.Ready++; else r.Pending++; }
    return r;
  }, [data]);
...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("apps", "replicasets", { listParams, namespace: nsFilter === "all" ? undefined : nsFilter });
  const { data: metricsData } = useMetrics("/api/k8s/metrics/workloads?kind=replicasets");
  ...
  const columns = React.useMemo(() => [...replicaSetColumns, ...metricsColumns], [metricsColumns]);
  ...
  const kubectlCmd = nsFilter === "all"
    ...
    : `kubectl get replicasets -n ${nsFilter}`;

  return (
    <div className="px-4 sm:px-6 py-5">
      <PageHeader title="ReplicaSets" count={pagination?.totalItems} subtitle="apps/v1 · all namespaces" kubectlCmd={kubectlCmd} resourceKey="replicasets"><StatusSummary data={data} /></PageHeader>
