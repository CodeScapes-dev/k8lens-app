...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("apps", "replicasets", { listParams });
  ...
  return (
    <div className="px-4 sm:px-6 py-5">
      <PageHeader title="ReplicaSets" count={pagination?.totalItems} subtitle="apps/v1 · all namespaces"><StatusSummary data={data} /></PageHeader>
      ...
      <DataTable columns={replicaSetColumns} data={data} loading={loading} refreshing={refreshing} pagination={pagination} listParams={listParams} onParamsChange={setListParams} filterChips={<FilterChip label="Namespace" value={nsFilter} onChange={setNsFilter} options={namespaces} />} footerText="Live · watching apps/v1 · replicasets" viewMode={viewMode} onViewModeChange={setViewMode} onRowClick={(r) => router.push(`/workloads/replicasets/${r.metadata.namespace}/${r.metadata.name}`)} />
    </div>
  );
}
