...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("batch", "cronjobs", { listParams });
  ...
  return (
    <div className="px-4 sm:px-6 py-5">
      ...
      <DataTable columns={cronJobColumns} data={data} loading={loading} refreshing={refreshing} pagination={pagination} listParams={listParams} onParamsChange={setListParams} filterChips={<FilterChip label="Namespace" value={nsFilter} onChange={setNsFilter} options={namespaces} />} footerText="Live · watching batch/v1 · cronjobs" viewMode={viewMode} onViewModeChange={setViewMode} onRowClick={(r) => router.push(`/workloads/cronjobs/${r.metadata.namespace}/${r.metadata.name}`)} />
    </div>
  );
}
