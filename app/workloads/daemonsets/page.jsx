...
import { daemonSetColumns } from "@/lib/k8s/columns/apps";
...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("apps", "daemonsets", { listParams, namespace: nsFilter === "all" ? undefined : nsFilter });
  const { data: metricsData } = useMetrics("/api/k8s/metrics/workloads?kind=daemonsets");
  ...
  const columns = React.useMemo(() => [...daemonSetColumns, ...metricsColumns], [metricsColumns]);
  ...
  const kubectlCmd = nsFilter === "all"
    ...
    : `kubectl get daemonsets -n ${nsFilter}`;

  return (
    <div className="px-4 sm:px-6 py-5">
      <PageHeader title="DaemonSets" count={pagination?.totalItems} subtitle="apps/v1 · all namespaces" kubectlCmd={kubectlCmd} resourceKey="daemonsets"><StatusSummary data={data} /></PageHeader>
