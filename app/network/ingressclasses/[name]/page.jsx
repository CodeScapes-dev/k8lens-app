...
export default function IngressClassDetailPage() {
  ...
  const { data, loading, error, refresh } = useK8sDetail("ingressclass", null, name);
  const diagCount = data ? diagnoseResource("ingressclass", data).length : 0;
  const [activeTab, setActiveTab] = React.useState("Overview");

  const ic = data?.ingressClass ?? null;
    ...
    <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              ...
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-2 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[180px] sm:min-w-0">
                  <QuickStat label="Default" value={isDefault ? "Yes" : "No"} />
                  <QuickStat label="Age" value={ic?.metadata?.creationTimestamp ? calculateAge(ic.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        <DiagnosticsPrompt
          ...
          resourceLabel="Ingress Class"
          data={data}
          uid={data?.ingressClass?.metadata?.uid}
          activeTab={activeTab}
          onViewDiagnostics={() => setActiveTab("Diagnostics")}
        />
        {activeTab === "Diagnostics" && <DiagnosticsPanel resourceType="ingressclass" data={data} showEmpty />}
        {activeTab === "Overview" && ic && (
          <Panel title="Info">
            <div className="grid gap-y-2 gap-x-3" style={{ gridTemplateColumns: "minmax(100px, 160px) 1fr", fontSize: 12 }}>
              {[
                ...
                ["UID", ic.metadata?.uid],
                ["Created", ic.metadata?.creationTimestamp ? calculateAge(ic.metadata.creationTimestamp) + " ago" : "—"],
              ].map(([l, v]) => (
