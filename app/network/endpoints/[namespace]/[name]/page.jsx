...
import { OverviewTab } from "@/components/endpoints-detail/tabs/OverviewTab";
...
export default function EndpointsDetailPage() {
  const { namespace, name } = useParams();
  const { data, loading, error, refresh } = useK8sDetail("endpoints", namespace, name);
  const diagCount = data ? diagnoseResource("endpoints", data).length : 0;
  const [activeTab, setActiveTab] = React.useState("Overview");

  const ep = data?.endpoints ?? null;
  const events = data?.events ?? [];
  const totalReady = (ep?.subsets ?? []).flatMap((s) => s.addresses ?? []).length;
    ...
    <div>
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              <div className="min-w-0 flex-1 space-y-2">
                <span className="font-mono text-xs text-muted-foreground">Endpoints · v1 · {namespace}</span>
              ...
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-2 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[180px] sm:min-w-0">
                  <QuickStat label="Ready" value={totalReady} />
                  <QuickStat label="Age" value={ep?.metadata?.creationTimestamp ? calculateAge(ep.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        <DiagnosticsPrompt
          ...
          data={data}
          uid={data?.endpoints?.metadata?.uid}
          activeTab={activeTab}
          onViewDiagnostics={() => setActiveTab("Diagnostics")}
        />
        {activeTab === "Diagnostics" && <DiagnosticsPanel resourceType="endpoints" data={data} showEmpty />}
