...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("rbac", "roles", { listParams });
  ...
  return (
    <div className="px-4 sm:px-6 py-5">
      ...
      <DataTable columns={roleColumns} data={data} loading={loading} refreshing={refreshing} pagination={pagination} listParams={listParams} onParamsChange={setListParams} filterChips={<FilterChip label="Namespace" value={nsFilter} onChange={setNsFilter} options={namespaces} />} footerText="Live · watching rbac/v1 · roles" viewMode={viewMode} onViewModeChange={setViewMode} onRowClick={(r) => router.push(`/access-control/roles/${r.metadata.namespace}/${r.metadata.name}`)} />
    </div>
  );
}
