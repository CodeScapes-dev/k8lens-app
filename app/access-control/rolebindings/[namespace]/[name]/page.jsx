...
export default function RoleBindingDetailPage() {
  ...
  const { data, loading, error, refresh } = useK8sDetail("rolebinding", namespace, name);
  const diagCount = data ? diagnoseResource("rolebinding", data).length : 0;
  const [activeTab, setActiveTab] = React.useState("Overview");

  const rb = data?.roleBinding ?? null;
  const role = data?.role ?? null;
  const events = data?.events ?? [];
  const subjects = rb?.subjects ?? [];
  const rules = role?.rules ?? [];
  const isClusterAdmin = rb?.roleRef?.name === "cluster-admin";
    ...
    <div>
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              <div className="min-w-0 flex-1 space-y-2">
                <span className="font-mono text-xs text-muted-foreground">Role Binding · rbac.authorization.k8s.io/v1 · {namespace}</span>
              ...
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-3 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[220px] sm:min-w-0">
                  ...
                  <QuickStat label="Age" value={rb?.metadata?.creationTimestamp ? calculateAge(rb.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        <DiagnosticsPrompt
          ...
          resourceLabel="Role Binding"
          data={data}
          uid={data?.roleBinding?.metadata?.uid}
          activeTab={activeTab}
          onViewDiagnostics={() => setActiveTab("Diagnostics")}
        />
        {activeTab === "Diagnostics" && <DiagnosticsPanel resourceType="rolebinding" data={data} showEmpty />}
        {activeTab === "Overview" && rb && (
          <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-4 items-start">
            <div className="flex flex-col gap-4">
              <Panel title="RoleBinding Info">
                <div className="grid gap-y-2 gap-x-3" style={{ gridTemplateColumns: "minmax(80px, 130px) 1fr", fontSize: 12 }}>
                  {[["Name", rb.metadata?.name], ["Namespace", rb.metadata?.namespace], ["UID", rb.metadata?.uid], ["Created", rb.metadata?.creationTimestamp ? calculateAge(rb.metadata.creationTimestamp) + " ago" : "—"]].map(([l, v]) => (
              ...
              <Panel title="Subjects" subtitle={`${subjects.length} subject${subjects.length !== 1 ? "s" : ""}`}>
                {subjects.length === 0 ? (
                  ...
                  subjects.slice(0, 5).map((s, i) => {
                    const href = s.kind === "ServiceAccount" ? `/access-control/serviceaccounts/${s.namespace ?? namespace}/${s.name}` : null;
                    const inner = (
                      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderTop: "1px solid var(--kl-border)" }}>
                        ...
                        {s.namespace && s.namespace !== namespace && <span style={{ fontSize: 11, color: "var(--kl-text-faint)" }}>{s.namespace}</span>}
                      </div>
                    );
            ...
            <div className="flex flex-col gap-4">
              <Panel title="Role Reference">
                <div className="flex items-center gap-2 flex-wrap">
                  <KLBadge tone={isClusterAdmin ? "err" : "accent"}>{rb.roleRef?.kind}</KLBadge>
                  <span className="kl-mono break-all" style={{ fontSize: 13 }}>{rb.roleRef?.name}</span>
                </div>
                {isClusterAdmin && (
        ...
        {activeTab === "Subjects" && <SubjectsTab subjects={subjects} defaultNamespace={namespace} />}
        ...
        {activeTab === "Dependencies" && <DependencyGraph resourceType="rolebinding" resource={rb} />}
        {activeTab === "Events" && <SharedEventsTab events={events} />}
        {activeTab === "Metadata" && <SharedMetadataTab resource={rb} />}
      </div>
    </div>
  );
}
