...
import { serviceAccountColumns } from "@/lib/k8s/columns/rbac";
...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("core", "serviceaccounts", { listParams });
  ...
  return (
    <div className="px-4 sm:px-6 py-5">
      ...
      <DataTable columns={serviceAccountColumns} data={data} loading={loading} refreshing={refreshing} pagination={pagination} listParams={listParams} onParamsChange={setListParams} filterChips={<FilterChip label="Namespace" value={nsFilter} onChange={setNsFilter} options={namespaces} />} footerText="Live · watching v1 · serviceaccounts" onRowClick={(r) => router.push(`/access-control/serviceaccounts/${r.metadata.namespace}/${r.metadata.name}`)} viewMode={viewMode} onViewModeChange={setViewMode} />
    </div>
  );
}
