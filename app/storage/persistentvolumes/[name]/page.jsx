...
import { OverviewTab, pvStatusKind } from "@/components/pv-detail/tabs/OverviewTab";
  ...
  return (
    <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {pv && <Badge variant="secondary" className="font-mono text-[10.5px] font-normal">{phase}</Badge>}
              ...
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-3 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[240px] sm:min-w-0">
                  ...
                  <QuickStat label="Capacity" value={spec.capacity?.storage ?? "—"} />
                  <QuickStat label="Age" value={pv?.metadata?.creationTimestamp ? calculateAge(pv.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        ...
        {activeTab === "Metadata" && <SharedMetadataTab resource={pv} />}
      </div>
    </div>
  );
}
