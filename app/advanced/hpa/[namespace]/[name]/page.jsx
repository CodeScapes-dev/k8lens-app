...
import { LayoutDashboardIcon, BellIcon, TagIcon, ActivityIcon, ShareIcon, StethoscopeIcon } from "lucide-react";
...
import { OverviewTab } from "@/components/hpa-detail/tabs/OverviewTab";
import { ConditionsTab } from "@/components/hpa-detail/tabs/ConditionsTab";
...
export default function HPADetailPage() {
  ...
  const { data, loading, error, refresh } = useK8sDetail("hpa", namespace, name);
  const diagCount = data ? diagnoseResource("hpa", data).length : 0;
  const [activeTab, setActiveTab] = React.useState("Overview");

  const hpa = data?.hpa ?? null;
  ...
  const scaleKind = current === desired ? "ok" : "warn";
  ...
  return (
    <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {hpa?.spec?.scaleTargetRef && (
                    <Badge variant="secondary" className="font-mono text-[10.5px] font-normal">{hpa.spec.scaleTargetRef.kind}: {hpa.spec.scaleTargetRef.name}</Badge>
                  )}
                </div>
              </div>
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-4 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[280px] sm:min-w-0">
                  <QuickStat label="Current" value={current || "—"} />
                  <QuickStat label="Desired" value={desired || "—"} />
                  <QuickStat label="Max" value={hpa?.spec?.maxReplicas ?? "—"} />
                  <QuickStat label="Age" value={hpa?.metadata?.creationTimestamp ? calculateAge(hpa.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        <DiagnosticsPrompt
          resourceType="hpa"
          resourceLabel="Autoscaler"
          data={data}
          uid={data?.hpa?.metadata?.uid}
          activeTab={activeTab}
          onViewDiagnostics={() => setActiveTab("Diagnostics")}
        />
        {activeTab === "Diagnostics" && <DiagnosticsPanel resourceType="hpa" data={data} showEmpty />}
