...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("autoscaling", "horizontalpodautoscalers", { listParams });
  ...
  return (
    <div className="px-4 sm:px-6 py-5">
      ...
      <DataTable columns={hpaColumns} data={data} loading={loading} refreshing={refreshing} pagination={pagination} listParams={listParams} onParamsChange={setListParams} filterChips={<FilterChip label="Namespace" value={nsFilter} onChange={setNsFilter} options={namespaces} />} footerText="Live · watching autoscaling/v2 · horizontalpodautoscalers" onRowClick={(r) => router.push(`/advanced/hpa/${r.metadata.namespace}/${r.metadata.name}`)} viewMode={viewMode} onViewModeChange={setViewMode} />
    </div>
  );
}
