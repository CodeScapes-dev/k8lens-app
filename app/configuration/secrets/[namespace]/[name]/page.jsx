...
import { OverviewTab } from "@/components/secret-detail/tabs/OverviewTab";
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              <div className="min-w-0 flex-1 space-y-2">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs text-muted-foreground">Secret · v1 · {namespace}</span>
                  {secret && <HealthBadge resourceType="secret" data={data} />}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <Badge variant="secondary" className="font-mono text-[10.5px] font-normal">{secret?.type ?? "Opaque"}</Badge>
                </div>
              </div>
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-3 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[240px] sm:min-w-0">
                  ...
                  <QuickStat label="Age" value={createdAt ? calculateAge(createdAt) : "—"} />
                  <QuickStat label="Rotation" value={ageDays > 180 ? ">180d" : ageDays > 90 ? ">90d" : "OK"} />
                </div>
              </div>
            </div>
