...
import { configMapColumns } from "@/lib/k8s/columns/core";
...
export default function Page() {
  ...
  const { data, loading, refreshing, error, pagination } = useK8sResource("core", "configmaps", { listParams, namespace: nsFilter === "all" ? undefined : nsFilter });
  ...
  const kubectlCmd = nsFilter === "all"
    ...
    : `kubectl get configmaps -n ${nsFilter}`;

  return (
    <div className="px-4 sm:px-6 py-5">
      <PageHeader title="ConfigMaps" count={pagination?.totalItems} subtitle="v1 · core · all namespaces"  kubectlCmd={kubectlCmd} resourceKey="configmaps" />
      ...
      <DataTable
        ...
        footerText="Live · watching v1 · configmaps" resourceKind="ConfigMap"
        onRowClick={(r) => router.push(`/configuration/configmaps/${r.metadata.namespace}/${r.metadata.name}`)}
        viewMode={viewMode}
