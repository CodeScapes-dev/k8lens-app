...
import { LayoutDashboardIcon, FileTextIcon, GitCompareIcon, BellIcon, TagIcon, ShareIcon, StethoscopeIcon } from "lucide-react";
...
import { ChangesTab } from "@/components/diff-viewer/ChangesTab";
import { OverviewTab } from "@/components/configmap-detail/tabs/OverviewTab";
import { DataTab } from "@/components/configmap-detail/tabs/DataTab";
...
export default function ConfigMapDetailPage() {
  ...
  const { data, loading, error, refresh } = useK8sDetail("configmap", namespace, name);
  const diagCount = data ? diagnoseResource("configmap", data).length : 0;
  const [activeTab, setActiveTab] = React.useState("Overview");

  const cm = data?.configMap ?? null;
    ...
    <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              ...
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-2 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[180px] sm:min-w-0">
                  <QuickStat label="Keys" value={keys.length} />
                  <QuickStat label="Age" value={cm?.metadata?.creationTimestamp ? calculateAge(cm.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        <DiagnosticsPrompt
          ...
          data={data}
          uid={data?.configMap?.metadata?.uid}
          activeTab={activeTab}
          onViewDiagnostics={() => setActiveTab("Diagnostics")}
        />
        {activeTab === "Diagnostics" && <DiagnosticsPanel resourceType="configmap" data={data} showEmpty />}
