...
import { OverviewTab } from "@/components/resourcequota-detail/tabs/OverviewTab";
...
export default function ResourceQuotaDetailPage() {
  ...
  const { data, loading, error, refresh } = useK8sDetail("resourcequota", namespace, name);
  const diagCount = data ? diagnoseResource("resourcequota", data).length : 0;
  const [activeTab, setActiveTab] = React.useState("Overview");

  const rq = data?.resourceQuota ?? null;
    ...
    <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              ...
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-2 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[180px] sm:min-w-0">
                  <QuickStat label="Constraints" value={Object.keys(hard).length} />
                  <QuickStat label="Age" value={rq?.metadata?.creationTimestamp ? calculateAge(rq.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        <DiagnosticsPrompt
          ...
          resourceLabel="Resource Quota"
          data={data}
          uid={data?.resourceQuota?.metadata?.uid}
          activeTab={activeTab}
          onViewDiagnostics={() => setActiveTab("Diagnostics")}
        />
        {activeTab === "Diagnostics" && <DiagnosticsPanel resourceType="resourcequota" data={data} showEmpty />}
