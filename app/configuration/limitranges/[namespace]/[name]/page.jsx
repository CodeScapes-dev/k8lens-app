...
import { LayoutDashboardIcon, BellIcon, TagIcon, ShareIcon, StethoscopeIcon } from "lucide-react";
...
import { OverviewTab } from "@/components/limitrange-detail/tabs/OverviewTab";
...
export default function LimitRangeDetailPage() {
  ...
  const { data, loading, error, refresh } = useK8sDetail("limitrange", namespace, name);
  const diagCount = data ? diagnoseResource("limitrange", data).length : 0;
  const [activeTab, setActiveTab] = React.useState("Overview");

  const lr = data?.limitRange ?? null;
    ...
    <div>
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
              <div className="min-w-0 flex-1 space-y-2">
                <span className="font-mono text-xs text-muted-foreground">Limit Range · v1 · {namespace}</span>
              ...
              <div className="overflow-x-auto w-full sm:w-auto shrink-0" style={{ scrollbarWidth: "none" }}>
                <div className="grid grid-cols-2 rounded-xl border border-border overflow-hidden divide-x divide-border min-w-[180px] sm:min-w-0">
                  <QuickStat label="Types" value={limits.length} />
                  <QuickStat label="Age" value={lr?.metadata?.creationTimestamp ? calculateAge(lr.metadata.creationTimestamp) : "—"} />
                </div>
              </div>
            </div>
      ...
      <div className="px-4 sm:px-7 py-5">
        <DiagnosticsPrompt
          ...
          resourceLabel="Limit Range"
          data={data}
          uid={data?.limitRange?.metadata?.uid}
          activeTab={activeTab}
          onViewDiagnostics={() => setActiveTab("Diagnostics")}
        />
        {activeTab === "Diagnostics" && <DiagnosticsPanel resourceType="limitrange" data={data} showEmpty />}
        {activeTab === "Overview" && <OverviewTab limitRange={lr} />}
        {activeTab === "Dependencies" && <DependencyGraph resourceType="limitrange" resource={lr} />}
        {activeTab === "Events" && <SharedEventsTab events={events} />}
        {activeTab === "Metadata" && <SharedMetadataTab resource={lr} />}
      </div>
    </div>
  );
}
