...
export function ResourcesTab({ pods }) {
  return (
    <Panel title="Pods Mounting This Claim" subtitle={pods.length > 0 ? `${pods.length} pod${pods.length !== 1 ? "s" : ""}` : undefined}>
      {pods.length === 0 ? (
        <div style={{ fontSize: 12, color: "var(--kl-text-muted)" }}>No pods are currently mounting this claim.</div>
      ) : (
