...
export function SharedMetadataTab({ resource }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {owners.length > 0 && (
        <Panel title="Owner References">
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {owners.map((o) => (
              <div key={o.uid} style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 8, fontSize: 12 }}>
                <span style={{ color: "var(--kl-text-muted)" }}>{o.kind}</span>
                <span className="kl-mono">{o.name}</span>
              </div>
            ))}
          </div>
        </Panel>
      ...
      <Panel title="System">
        <div style={{ display: "grid", gridTemplateColumns: "minmax(100px, 160px) 1fr", gap: "8px 12px" }}>
          <span style={{ color: "var(--kl-text-muted)", fontSize: 12 }}>UID</span>
          <span className="kl-mono" style={{ fontSize: 11.5 }}>{meta.uid ?? "—"}</span>
