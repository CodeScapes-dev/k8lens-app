...
export function DeploymentLogsTab({ pods = [] }) {
  const activeContext = useClusterStore((s) => s.activeContext);
  const clusters = useClusterStore((s) => s.clusters);

  const podNames = pods.map((p) => p.metadata?.name).filter(Boolean);
  const [selectedPod, setSelectedPod] = React.useState(podNames[0] ?? "");

  const selectedPodObj = pods.find((p) => p.metadata?.name === selectedPod);
  const allContainers = [
    ...(selectedPodObj?.spec?.initContainers ?? []).map((c) => ({ ...c, init: true })),
    ...(selectedPodObj?.spec?.containers ?? []),
  ];
  ...
  const namespace = selectedPodObj?.metadata?.namespace;

  const fetchLogs = React.useCallback(async () => {
    if (!namespace || !selectedPod || !activeContext) return;
    setLoading(true);
    setLostConnection(false);
    try {
      const cluster = clusters.find((c) => c.contextName === activeContext);
      const params = new URLSearchParams({
        namespace, pod: selectedPod, context: activeContext,
        timestamps: "false",
      });
  ...
  }, [namespace, selectedPod, activeContext, clusters, container, tailLines, sinceSeconds, live]);
  ...
  const copyLogs = () => { navigator.clipboard.writeText(logs).catch(() => {}); };

  const downloadLogs = () => {
    const ts = new Date().toISOString().replace(/[:.]/g, "-");
    const fname = `${selectedPod}-${container}-${ts}.txt`;
  ...
  const containerStatus = (cName) => {
    const all = [
      ...(selectedPodObj?.status?.initContainerStatuses ?? []),
      ...(selectedPodObj?.status?.containerStatuses ?? []),
    ];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 0, height: "calc(100vh - 320px)", minHeight: 320 }}>
      ...
      <div style={{
        ...
        <Select value={selectedPod} onValueChange={setSelectedPod}>
          <SelectTrigger style={{ width: 220, height: 30, fontSize: 12 }}>
            <SelectValue placeholder="Select pod" />
          </SelectTrigger>
          <SelectContent>
            {podNames.map((pName) => {
              const p = pods.find((x) => x.metadata?.name === pName);
        ...
        <Select value={container} onValueChange={setContainer}>
          <SelectTrigger style={{ width: 160, height: 30, fontSize: 12 }}>
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {allContainers.map((c) => {
        ...
        <input
          ...
          style={{
            ...
            color: "var(--kl-text)", outline: "none", width: 150,
          }}
        />
      ...
      >
        ...
        {lines.map((line, i) => (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 0, minHeight: 20 }}>
            <span style={{
