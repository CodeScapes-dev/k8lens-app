...
export function MetadataTab({ deployment }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Panel title="Template Labels" subtitle="pod template">
        {(() => {
          const tl = deployment.spec?.template?.metadata?.labels ?? {};
          return Object.keys(tl).length === 0 ? (
            <span style={{ fontSize: 12, color: "var(--kl-text-muted)" }}>No template labels</span>
          ) : (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
      ...
      <Panel title="System">
        <div style={{ display: "grid", gridTemplateColumns: "minmax(100px, 160px) 1fr", gap: "8px 12px" }}>
          ...
          <span style={{ color: "var(--kl-text-muted)", fontSize: 12 }}>Observed Generation</span>
          <span className="kl-mono" style={{ fontSize: 12 }}>{deployment.status?.observedGeneration ?? "—"}</span>
