...
function EnvTable({ containers }) {
  const withEnv = containers.filter((c) => c.env?.length > 0);
  if (!withEnv.length) return null;
  return withEnv.map((c) => (
...
export function ResourcesTab({ deployment }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <ContainersTable containers={containers} title="Containers" />
      <ContainersTable containers={initContainers} title="Init Containers" />
      <EnvTable containers={[...containers, ...initContainers]} />
